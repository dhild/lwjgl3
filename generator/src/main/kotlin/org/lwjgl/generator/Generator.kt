...
fun main(args: Array<String>) {
	...
	Generator(args[0], args[1]) {
		...
		args.drop(2).forEach { generate(it) }
...
//		generate("org.lwjgl.system.windows")
		...
		generate("struct", Generator.structs.filter { struct -> args.drop(2).any { struct.packageName.contains(it) } })
		generate("callback", Generator.callbacks.filter { struct -> args.drop(2).any { struct.packageName.contains(it) } })
		generate("custom class", Generator.customClasses.filter { struct -> args.drop(2).any { struct.packageName.contains(it) } })
	}
}
